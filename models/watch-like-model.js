...
const WatchLikesModel = {
    getWatchLikesByUserID: async function (profile) {
        try {
            ...
            WHERE Watchlists.list_name = 'Beğenilenler' AND Watchlists.user_id = @user_id
            ORDER BY 
            NEWID();     
            SELECT COUNT(*) AS TotalCountLike
            FROM (
                SELECT 
                    Watchlists.watch_list_id,
                    Watchlists.user_id,
                ...
                if (result.recordsets && result.recordsets.length === 2) {
                    ...
                    const totalCountLike = result.recordsets[1][0].TotalCountLike;
                    return {
                        videos,
                        totalCountLike
                    };
...
module.exports = WatchLikesModel;